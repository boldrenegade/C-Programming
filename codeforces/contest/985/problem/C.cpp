...
// dbg(var)
#define dbg(...) cerr << "[" << #__VA_ARGS__ << "]: "<<to_string(__VA_ARGS__)<<"\n";
...
inline int total(int n) { return int((-1 + sqrt(1 + 8 * n)) / 2); }
const int N = 1e5;
...
void solve() {
    ...
    FOR(i, 0, n*k) cin >> a[i];
    vsort(a);
    ll need = upper_bound(a.begin(), a.end(), a[0]+l) - a.begin();
    if(need<n){
        cout<<0;
        return;
    }
...
int main() {   
    ...
    clock_t stop = clock();
    double elapsed = (double) 1000 * (stop - start) / CLOCKS_PER_SEC;
